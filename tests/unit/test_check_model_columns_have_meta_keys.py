...
import pytest
from dbt_checkpoint.check_model_columns_have_meta_keys import (
    main,
    check_column_meta_keys,
)
...
-   name: in_schema_some_column_meta
    ...
    yml_file = tmpdir.join(f"schema.{extension}")
    yml_file.write(schema_yml)
    res_stat, missing = check_column_meta_keys(
    ...
    assert res_stat == 1
    assert missing == {
        "in_schema_some_column_meta": {"test1": {"bar"}, "test2": {"foo", "bar"}}
    }
